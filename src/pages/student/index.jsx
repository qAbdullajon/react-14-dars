...
export default function CustomizedTables() {
  ...
  const [guruhs, setGuruhs] = React.useState([]);
  ...
  const handleEdit = (student) => {
    setForm(student);
    setOpen(true);
    setAge(student.courseName);
    setUpdate(true);
  };
  const putStudent = async (student, id) => {
    try {
      const res = await axios.put(`http://localhost:3000/student/${id}`, student);
      if (res.status === 200) {
        getStudent();
      }
    } catch (err) {
  ...
  return (
    <div>
      <StudentModal courses={courses} guruhs={guruhs} teachers={teachers} open={open} setOpen={setOpen} form={form} setForm={setForm} postStudent={postStudent} update={update} putStudent={putStudent} age={age} setAge={setAge} />
        ...
        <Table sx={{ minWidth: 700 }} aria-label="customized table">
          <TableHead sx={{ backgroundColor: "black" }}>
            <TableRow>
              ...
              <StyledTableCell align="left">Student full name</StyledTableCell>
              <StyledTableCell align="left">Student age</StyledTableCell>
              <StyledTableCell align="left">Phonen number</StyledTableCell>
            ...
            {data.map((item, i) => (
              <StyledTableRow key={i} sx={{ backgroundColor: i % 2 === 0 ? "#555 !important" : "#333" }}>
                ...
                <StyledTableCell align="left">
                  <span className="text-white">{item.studentFullName}</span>
                </StyledTableCell>
                <StyledTableCell align="left">
                  <span className="text-white">{item.studentAge}</span>
                </StyledTableCell>
                <StyledTableCell align="left">
